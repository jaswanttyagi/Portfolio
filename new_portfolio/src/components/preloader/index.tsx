...
function Preloader({ children, disabled = false }: PreloaderProps) {
  ...
  useEffect(() => {
    ...
    // The portfolio must never stay covered when an optional visual asset (such
    // as the Spline scene) is unavailable. GSAP drives the progress animation,
    // while this native timer guarantees that the content is revealed.
